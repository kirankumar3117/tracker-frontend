...
export function HabitMatrix({ habits, onToggleLog, loadingHabitId, selectedMonth, setSelectedMonth, selectedYear, setSelectedYear }: HabitMatrixProps) {
  ...
  const [timeTravelerToast, setTimeTravelerToast] = useState(false);

  const handleCellClick = (habitId: string, day: Date, isCompleted: boolean) => {
    const clickedDay = new Date(day);
    clickedDay.setHours(0, 0, 0, 0);
    if (clickedDay.getTime() > today.getTime()) {
      setTimeTravelerToast(true);
      setTimeout(() => setTimeTravelerToast(false), 3000);
      return;
    }
    onToggleLog(habitId, day, isCompleted);
  };
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
                      ...
                      return (
                        <div key={day.toISOString()} className={`w-14 shrink-0 flex items-center justify-center border-r border-border h-12 snap-center ${isToday ? 'bg-primary/5' : ''}`}>
                          <motion.button
                            ...
                            onClick={() => isActive && handleCellClick(habit.id, day, !isCompleted)}
                            className={`w-6 h-6 rounded flex items-center justify-center transition-all ${isLoading ? 'opacity-50' : ''
      ...
      <AnimatePresence>
        {timeTravelerToast && (
          <motion.div
            key="time-traveler-toast"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            transition={{ type: "spring", stiffness: 300, damping: 25 }}
            className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[9999] bg-[#0d1117] border border-orange-500/30 text-slate-200 rounded-xl shadow-2xl px-4 py-3 flex items-center gap-3 max-w-sm w-full"
          >
            <span className="text-2xl">⏱️</span>
            <p className="text-sm font-medium leading-snug">
              Whoa there, <span className="text-orange-400 font-bold">Time Traveler!</span> Tomorrow hasn&apos;t even happened yet. Let&apos;s finish today first, bro.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
